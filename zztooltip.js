...
(function() {
  ...
  const CSS = `
    ...
    /* Modal mode (small viewports): center the tooltip with a backdrop
       instead of trying to position next to the tap point. Tap is added
       at runtime by tooltip.js; backdrop is its own element below. */
    #kcraft-tooltip.tt-modal {
      ...
      overflow-y: auto;
      pointer-events: auto;  /* allow internal scroll */
      z-index: 10001;
    }
  ...
  function setupEventListeners() {
    ...
    document.body.addEventListener('touchend', (ev) => {
      ...
      if (row) {
        ...
        // Don't dismiss on taps inside the tooltip itself — lets users
        // scroll long set-bonus lists without triggering close.
        if (ev.target.closest('#kcraft-tooltip')) return;
        hideTooltip();
      }
    }, { passive: false });
